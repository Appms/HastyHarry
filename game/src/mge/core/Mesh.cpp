...
Mesh* Mesh::load(string pFileName)
{
	...
	if( file.is_open() ){
				...
				if ( count == 10 ) {
					...
					for ( int i = 0; i < 3; ++i ) {
						...
						if (found == mappedTriplets.end())
                        {
							...
							//store vertex data in triangles structs
							triangle.index[i] = index;
						}
						else
                        {
		...
		cout << mappedTriplets.size() << endl;
		cout << mesh->_tris.size() << endl;

		file.close();

		mesh->_calculateTangentArray();
...
void Mesh::_calculateTangentArray() {

    int vertexCount = this->_vertices.size();

    vector<glm::vec3> tan1(vertexCount,glm::vec3(0.0f,0.0f,0.0f));
    vector<glm::vec3> tan2(vertexCount,glm::vec3(0.0f,0.0f,0.0f));

    for(int a = 0; a < this->_tris.size(); a++){
        int i1 = this->_tris[a].index[0];
        int i2 = this->_tris[a].index[1];
        int i3 = this->_tris[a].index[2];

        const glm::vec3 v1 = this->_vertices[i1];
        const glm::vec3 v2 = this->_vertices[i2];
        const glm::vec3 v3 = this->_vertices[i3];

        const glm::vec2 w1 = this->_uvs[i1];
        const glm::vec2 w2 = this->_uvs[i2];
        const glm::vec2 w3 = this->_uvs[i3];
    ...
    for(int a=0; a < vertexCount; a++){
        const glm::vec3& n = this->_normals[a];
        const glm::vec3& t = tan1[a];

        //Gram-Schmidt orthogonalize
        glm::vec3 finalTan = glm::normalize(t - n * glm::dot(n,t));

        //Calculate handedness
        float w = (glm::dot(glm::cross(n,t), tan2[a]) < 0.0f) ? -1.0f : 1.0f;

        this->_tangents.push_back(glm::vec4(finalTan, w));
    }

    cout << "Tangents calculated!" << endl;
}
...
void Mesh::streamToOpenGL(GLint pVerticesAttrib, GLint pNormalsAttrib, GLint pUVsAttrib, GLint pTangentsAttrib) {
    ...
    if(pTangentsAttrib != -1) {
        ...
        glVertexAttribPointer(pTangentsAttrib, 4, GL_FLOAT, GL_TRUE, 0, 0 );
    }
